...
class SynthNode(Node):
    ...
    def tick(self,t):
        if len(self.out_port.edges) == 0:
            return
        if t - self.sample_t > self.timeout: # Too much time has passed since last sample. Skip to current time
...
class BasicSynthNode(SynthNode):
    ...
    def build_node(self):
        super().build_node()
        self.waveform = self.add_control(name='waveform',label='waveform', control_type=OptionControl, options=['sine', 'square', 'sawtooth', 'triangle'], value='sine')

    def sample(self, n:int, l:int):
        sample_rate = self.ext.sample_rate
        if self.waveform.get() == 'sine':
            return np.sin(2 * math.pi * self.freq * np.arange(n, n + l) / sample_rate, dtype='float32')
        elif self.waveform.get() == 'square':
            return np.sign(np.sin(2 * math.pi * self.freq * np.arange(n, n + l) / sample_rate), dtype='float32')*0.1
        elif self.waveform.get() == 'sawtooth':
            return 2 * (self.freq * np.arange(n, n + l, dtype='float32') / sample_rate - np.floor(0.5 + self.freq * np.arange(n, n + l, dtype='float32') / sample_rate))*0.1
        elif self.waveform.get() == 'triangle':
            return 2 * np.abs(2 * (self.freq * np.arange(n, n + l, dtype='float32') / sample_rate - np.floor(0.5 + self.freq * np.arange(n, n + l, dtype='float32') / sample_rate))) - 1
...
class ADSRNode(SynthNode):
    ...
    def build_node(self):
        ...
        self.attack = self.add_control(name='attack',label='attack', control_type=TextControl ,text='1')
        self.decay = self.add_control(name='decay',label='decay', control_type=TextControl ,text='0.8')
        self.sustain = self.add_control(name='sustain',label='sustain', control_type=TextControl ,text='0.5')
        self.attack_time = self.add_control(name='attack_time',label='attack time', control_type=TextControl ,text='0.1')
        self.decay_time = self.add_control(name='decay_time',label='decay time', control_type=TextControl ,text='0.1')
        self.sustain_time = self.add_control(name='sustain_time',label='sustain time', control_type=TextControl ,text='0.5')
        self.release_time = self.add_control(name='release_time',label='release time', control_type=TextControl ,text='0.1')
    ...
    def get_on_envelope(self):
        ...
        decay_samples = np.linspace(attack, decay, int(decay_time * self.ext.sample_rate))
        # sustain is exponential decay
        sustain_samples = np.exp(np.linspace(0, np.log(sustain/decay), int(sustain_time * self.ext.sample_rate)))*decay
...
class InstrumentNode(SynthNode):
    ...
    def init_node(self):
        ...
        self.relase_envelope = np.concatenate([np.linspace(1, 0, int(self.release_time*self.ext.sample_rate), dtype='float32'),
    ...
    def edge_activated(self, edge: Edge, port: InputPort):
        ...
        if port == self.note_on_port:
            ...
            if pitch in self.playing_notes:
                self.playing_notes[pitch].offset = note_info.get('offset',int(self.sample_t*self.ext.sample_rate))
