...
class AudioOuputNode(Node):
    ...
    def build_node(self):
        ...
        self.gain_control = self.add_slider_control(
            name="gain", label="Gain", min=-5, max=5, step=0.01
        )
    ...
    def collect(self):
        for edge in self.samples_port.edges:
            if edge.is_activated():
                ...
                if len(self.buffer) > self.ext.sample_rate * 5:
                    self.print_exception("Buffer overflow")
                    self.buffer = byteFIFO()

        if not self.playing and len(self.buffer) > self.ext.sample_rate * 0.00:
            self.playing = True
    ...
    def callback(self, in_data, frame_count, time_info, status):
        ...
        if self.delay_update_i == 0:
            self.delay_control.set(
                f"{len(self.buffer)/4/self.ext.sample_rate*1000:.2f} ms"
            )
