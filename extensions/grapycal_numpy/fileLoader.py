...
class FileLoaderNode(Node):
    ...
    def create(self):
        ...
        self.label_topic.set("File Loader")
        ...
        self.option_control = self.add_option_control(
            options=["JSON", "NPM", "PKL", "TXT"], value="JSON", name="opt"
        )
    ...
    def button_clicked(self):
        ...
        if self.option_control.get() == "JSON":
            selected_files = filedialog.askopenfilenames(
                parent=root, initialdir="./", filetypes=[("json files", "*.json")]
            )
        if self.option_control.get() == "NPY":
            selected_files = filedialog.askopenfilenames(
                parent=root, initialdir="./", filetypes=[("numpy files", "*.npy")]
            )
        if self.option_control.get() == "PKL":
            selected_files = filedialog.askopenfilenames(
                parent=root, initialdir="./", filetypes=[("pickle files", "*.pkl")]
            )
        if self.option_control.get() == "TXT":
            selected_files = filedialog.askopenfilenames(
                parent=root, initialdir="./", filetypes=[("text files", "*.txt")]
            )
...
class FileFilterNode(Node):
    ...
    def create(self):
        self.label_topic.set("File Filter")
        self.shape_topic.set("simple")
        self.text = self.add_text_control("0")
