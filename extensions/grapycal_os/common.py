from grapycal import TextControl, ButtonControl, InputPort, Node, StringTopic
import subprocess
import os
...
class CommandNode(Node):
    ...
    def build_node(self):
        ...
        self.environment = self.add_attribute(
            "environment",
            StringTopic,
            editor_type="options",
            options=["default", "git bash"],
        )
    ...
    def port_activated(self, port: InputPort):
        if port == self.run_port or port == self.stdin_port:
            if (
                ...
                and self.stdin_port.is_all_ready()
            ):
                self.run(self.task)
    ...
    def task(self):
        ...
        result = subprocess.run(
            commands, shell=True, capture_output=True, text=True, cwd=cwd, input=stdin
        )
