...
class ZeroesNode(SourceNode):
    ...
    def task(self):
        self.out.push(
            torch.zeros(
                *map(int, self.shape_text.text.get().split(",")),
...
class GridNode(SourceNode):
    ...
    def restore_from_version(self, version: str, old: NodeInfo):
        ...
        self.restore_attributes(
            "x start", "x end", "x steps", "y start", "y end", "y steps", "device"
        )
        self.restore_controls("x_shape_text", "y_shape_text")

    def update_label_x(self, *args):
        self.x_shape_text.text.set(
            f"x: [{self.x_start.get()},{self.x_end.get()}] / {self.x_steps.get()}"
        )

    def update_label_y(self, *args):
        self.y_shape_text.text.set(
            f"y: [{self.y_start.get()},{self.y_end.get()}] / {self.y_steps.get()}"
        )
