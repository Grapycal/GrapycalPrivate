...
class LinearNode(SimpleModuleNode):
    ...
    def build_node(self, in_features=1, out_features=1):
        ...
        self.in_features = self.add_attribute(
            "in_features", IntTopic, in_features, editor_type="int"
        )
        self.out_features = self.add_attribute(
            "out_features", IntTopic, out_features, editor_type="int"
        )
...
class FlattenNode(SimpleModuleNode):
    ...
    def create_module(self) -> nn.Module:
        return nn.Flatten(start_dim=-3)  # TODO: make it configurable
