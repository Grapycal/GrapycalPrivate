...
class NetManager:
    ...
    def save_network(self, name, path):
        ...
        for mn in self.get_module_nodes(name):
            state_dicts[mn.state_dict_id.get()] = mn.get_torch_state_dict()
        ...
        for mn in self.get_module_nodes(name):
            if mn.state_dict_id.get() in state_dicts:
                try:
                    ...
                    raise Exception("Failed to load the network")
            else:
                mn.print_exception(
                    f"Warning: parameter for {mn.state_dict_id.get()} is missing in file {path}"
                )
        if len(state_dicts) > 0:
            load_node.print_exception(
                f"Warning: layers {list(state_dicts.keys())} are absent in the current network architecture, so they are not loaded"
            )
    ...
    def add_in(self, name: str, node: "NetworkInNode"):
        if not self.ext.has_command(f"Call: {name}"):
            self.ext.register_command(
                f"Call: {name}", lambda ctx: self._create_call(ctx, name)
            )
        self.ins[name] = node
