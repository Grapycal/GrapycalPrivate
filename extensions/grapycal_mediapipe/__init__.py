...
class PoseEstimateNode(Node):
    ...
    def build_node(self):
        super().build_node()
        self.label_topic.set("Pose Estimate")
        ...
        self.load_detector_button = self.add_button_control(
            "load detector", "load detector"
        )
    ...
    def load_detector(self):
        ...
        result_callback = lambda result, image, timestamp: self.run(
            self.result_callback, result=result, image=image, timestamp=timestamp
        )
    ...
    def task(self, image):
        if self.detector is None:
            raise Exception(
                "detector not loaded. click load detector button to load detector"
            )
        image = mp.Image(
            image_format=mp.ImageFormat.SRGB,
            data=(image.transpose(1, 2, 0) * 255).astype(np.uint8),
        )
