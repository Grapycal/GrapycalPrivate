...
class FuncInNode(Node):
    ...
    def build_node(self, name: str = "Function"):
        ...
        self.outs = self.add_attribute(
            "outs", ListTopic, editor_type="list", init_value=["x"]
        )
        ...
        self.func_name.add_validator(
            lambda x, _: x not in self.ext.func_def_manager.ins
        ...
        self.func_name.set(
            find_next_valid_name(self.func_name.get(), self.ext.func_def_manager.ins)
        )
    ...
    def on_output_set(self, new):
        if not self.is_preview.get():
            print(
                self.ext.func_def_manager.calls.get(self.func_name.get()),
                self.func_name.get(),
            )
            for call in self.ext.func_def_manager.calls.get(self.func_name.get()):
...
class FuncOutNode(Node):
    ...
    def build_node(self, name: str = "Function"):
        ...
        self.ins = self.add_attribute(
            "ins", ListTopic, editor_type="list", init_value=["y"]
        )
        ...
        self.func_name.add_validator(
            lambda x, _: x not in self.ext.func_def_manager.outs
        )
        self.func_name.add_validator(
            lambda x, _: x != ""
        )  # empty name may confuse users
        ...
        self.func_name.set(
            find_next_valid_name(self.func_name.get(), self.ext.func_def_manager.outs)
        )
