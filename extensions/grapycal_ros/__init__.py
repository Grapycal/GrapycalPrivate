...
class RosbridgeNode(Node):
    ...
    def build_node(self):
        ...
        self.subsciptions = self.add_attribute(
            "subsciptions", ListTopic, [], editor_type="list"
        )  # for user to manipulate
        self.subscription_type = self.add_attribute(
            "subscription type",
            StringTopic,
            "std_msgs/String",
            editor_type="options",
    ...
    def init_node(self):
        ...
        def link_subsciptions_to_subs_info():
            self.subsciptions.on_insert.add_auto(
                lambda topic_name, _: self.subs_info.add(
                    topic_name, self.subscription_type.get()
                )
            )
            self.subsciptions.on_pop.add_auto(
                lambda topic_name, _: self.subs_info.pop(topic_name)
            )
    ...
    def on_subscription_add2(self, topic_name, topic_type):
        if self.status != RosbridgeNode.Status.CONNECTED:
            return
        self.send(
            json.dumps({"op": "subscribe", "topic": topic_name, "type": topic_type})
        )
    ...
    def connect(self):
        ...
        main_store.clock.add_listener(self.on_tick, 0.1)
    ...
    def disconnect(self):
        self.ws.close()
        main_store.clock.remove_listener(self.on_tick)
