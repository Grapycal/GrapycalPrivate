...
class CodeControl(ValuedControl[str]):
    """
    ...
    def build(
        ...
        if readonly:
            editable = False
        self.text = self.add_attribute(
            "text",
            StringTopic,
            text,
            is_stateful=not readonly,
            order_strict=not readonly,
        )
        ...
        self.activation_mode = self.add_attribute(
            "activation_mode", IntTopic, activation_mode.value
        )
    ...
    def set_activation_callback(self, callback):
        ...
        elif self.activation_mode.get() == CodeControl.ActivationMode.ON_FINISH:
            self.on_finish += callback

        elif self.activation_mode.get() == CodeControl.ActivationMode.NO_ACTIVATION:
            pass
